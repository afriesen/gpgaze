...
% 	end
	...
	% Plot bars
	handles.bars = bar(barvalues, width,'edgecolor','k', 'linewidth', 2.75);
	hold on
...
% 		x = mean(x([1 3],:));
% 		handles.errors(i) = errorbar(x, barvalues(:,i), errors(:,i), 'k', ...
%       'linestyle', 'none', 'linewidth', 2);
% 		ymax = max([ymax; barvalues(:,i)+errors(:,i)]);
% 	end
    for i = 1:numbars 
        x = bsxfun(@plus, handles.bars(i).XData, [handles.bars(i).XOffset]'); 
        handles.errors(i) = errorbar(x, barvalues(:,i), errors(:,i), ...
...
% 	ylim([0 ymax*1.1]);
	...
	if ~isempty(bw_title)
		title(bw_title, 'fontsize',32);
	end
	if ~isempty(bw_xlabel)
		xlabel(bw_xlabel, 'fontsize',32);
	end
	if ~isempty(bw_ylabel)
		ylabel(bw_ylabel, 'fontsize',32, 'fontweight','bold');
    %ylabel(bw_ylabel, 'fontsize',32);
	end
	
	set(gca, 'xticklabel', groupnames, 'box', 'off', 'ticklength', [0 0], ...
    'fontsize', 26, 'xtick',1:numgroups, 'linewidth', 3,'xgrid','off','ygrid','off');
